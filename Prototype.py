...
class App:
    def __init__(self, master):
        ...
        self.check = Button(middleFrame, text='Check In', command=self.checkWindow, width=25, height=3)
        ...
        self.check.pack(padx=5, pady=30,)
    ...
    def checkWindow(self):
        self.newWindow = Toplevel(self.master)
        self.app = checkIn(self.newWindow)
...
class addCustomer:
    ...
    def addCustomer(self):
        ...
        cur.execute("""INSERT INTO sessionList
               (sessionID,customer_Firstname,customer_Surname,checked_In)
                VALUES (?,?,?,?)""", [ID, firstname, surname, 'N'])
        conn.commit()
...
class checkIn:
    ...
    def __init__(self, master):
        ...
        self.add = Button(master, text="Check In", command=self.checkIn)
        self.close = Button(master, text="Close Window", command=self.close)
        ...
        cur.execute('SELECT * FROM sessionList')
        data = cur.fetchall()
        print(data)
    ...
    def checkIn(self):
        ...
        cur.execute("""UPDATE sessionList
            SET checked_In = 'Y'
            WHERE
             sessionID = ? AND
        ...
        conn.commit()
        self.txt.insert(END, "Customer : %s %s, has checked in to session No.%d" % (firstname, surname, ID))
